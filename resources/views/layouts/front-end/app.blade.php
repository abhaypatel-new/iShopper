...
<script>
    ...
    function quickView(product_id) {
     ...
     function dlt(product_id){
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('delete-custom')}}",
                        method: 'POST',
                        data: {id: id},
                        success: function () {
                            toastr.success('{{\App\CPU\translate('Custom_product_deleted_Successfully.')}}');
                            location.reload();
                        }
                    });
                }
     ...
     function quickView12345(product_id) {
    // alert('ghgh');
        $.get({
     ...
     function quickViewgg(product_id) {

        $.get({
            url: '{{route('view_custom')}}',
            dataType: 'json',
            data: {
                product_id: product_id
            },
    ...
    // Custom AddTOCart
     ...
     function addToCartcustom(form_id = 'add-to-cart-form', redirect_to_checkout=false) {
        alert("hai");
        if (checkAddToCartValidity()) {
            $.ajaxSetup({
                headers: {
    ...
    function buy_now() {
     ...
     function buy_now_product() {
        alert("yes02");
        addToCartcustom('add-to-cart-form',true);
        /* location.href = "{{route('checkout-details')}}"; */
    }
    ...
    function updateCartQuantity(key) {
     ...
     function updateCartQuantitys(key) {
