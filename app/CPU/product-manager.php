...
class ProductManager
{
    ...
    {
        return Offer::with(['custom_product'])->where('custom_product_id', $id)->first();
    }
    ...
    public static function search_products($name, $limit = 10, $offset = 1)
    {
        ...
        $paginator = Product::with(['rating'])->where(function ($q) use ($key) {
     ...
     public static function search_products_by_seller($name, $id, $limit = 10, $offset = 1)
    {
       ...
       //  $key = [base64_decode($name)];
        //
        dd($key);
        // $paginator = Product::active()->with(['rating'])->where(['user_id' =>$id, 'added_by' => 'seller'])->where(
             $paginator = Product::with(['rating'])->where(['user_id' =>$id, 'added_by' => 'seller'])->where(
            function ($q) use ($key) {
            foreach ($key as $value) {
                $q->orWhere('name', 'like', "%{$value}%");
            }
