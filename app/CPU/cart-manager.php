...
class CartManager
{
   ...
   // Add To Cart Custom Product
       ...
       public static function add_to_cartcustom($request, $from_api = false)
    {
        ...
        $cart['seller_id'] = ($product->added_by == 'admin') ? 1 : $product->seller_id;
    ...
    public static function update_cart_qty($request)
    {
     ...
     public static function update_custom_cart_qty($request)
    {
        $user = Helpers::get_customer($request);
        $status = 1;
        $qty = 0;
        ...
        $product = CustomProduct::find($cart['custom_product_id']);
