...
class WebController extends Controller
{
    ...
    public function Custom_product($slug)
    {
      
        $product = CustomProduct::with(['reviews'])->where('id', $slug)->first();
        // dd($product);
        if ($product != null) {
            $countOrder = OrderDetail::where('custom_product_id', $product->id)->count();
            $countWishlist = Wishlist::where('custom_product_id', $product->id)->count();
            $relatedProducts = CustomProduct::with(['reviews'])->where('category_id', $product->category_id)->where('id', '!=', $product->id)->limit(12)->get();
            $deal_of_the_day = DealOfTheDay::where('custom_product_id', $product->id)->where('status', 1)->first();

            return view('web-views.products.custom-product-details', compact('product', 'countWishlist', 'countOrder', 'relatedProducts', 'deal_of_the_day'));
        }
