...
class LoginController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        if (isset($user) == false) {
            ...
            // $response['message'] = 'Offer has been added to the product1';
        ...
        if (isset($user) && $user->is_active && auth('customer')->attempt(['email' => $user->email, 'password' => $request->password], $remember)) {
            ...
            // $response['message'] = 'Offer has been added to the product..';
            // $response['data'] = auth()->user()->id;
            // return response()->json($response);
        }
    ...
    public function submit_customer(Request $request)
    {
        ...
        if (isset($user) && $user->is_active && auth('customer')->attempt(['email' => $user->email, 'password' => $request->password], $remember)) {
            ...
            $response['token'] =  $users->createToken('api-token')->accessToken;
            $response['message'] = 'Seller data retrived successfully';
            $response['data'] = $users;
            return response()->json($response);
