...
class ProductController extends Controller
{
    ...
    public function custom_status_update(Request $request)
    {

        if ($request['status'] == 0) {
            CustomProduct::where(['id' => $request['id'], 'added_by' => 'seller', 'user_id' => \auth('seller')->id()])->update([
                'status' => $request['status'],
            ]);
            return response()->json([
                'success' => 1,
            ...
            if (CustomProduct::find($request['id'])) {
                CustomProduct::where(['id' => $request['id']])->update([
                    'status' => $request['status'],
                ]);
                return response()->json([
                    'success' => 1,
    ...
    public function CCustomProductupdate(Request $request, $id)
    {
         ...
         $offer_data = Offer::where('custom_product_id', $id)->where('seller_id', \auth('seller')->id())->first(); 
        
         if($offer_data != null)
         {
            $offer_data->update(['seller_offer_id' => $request->seller_product_id]);
        }else{
            $offer_create = Offer::create([
                'custom_product_id' => $request->id,
                'seller_id' => \auth('seller')->id(),
                'seller_offer_id'=>$request->seller_product_id,
                'seller_offer_price'=>$request->seller_offer_price,
                'seller_descriptions' => $request->description,
                'request_status'=>1,
        ...
        if (!empty($data)) {
          ...
          $activated =    CustomProduct::where('id', $id)->update([ 'request_status'=>1]);  
           
           Toastr::success('Product updated successfully.');
            // return redirect()->back();
            return redirect()->route('seller.product.custom.list');
        } else {
    ...
    public function custom_view($id)
    {
        ...
        $offer = Offer::where(['custom_product_id' => $id])->where(['seller_id' => auth('seller')->id()])->first();
        ...
        return view('seller-views.custom-product.view', compact('product', 'offer', 'reviews','sellerproducts'));
    }
    ...
    //Custom Product Remove Image

    public function remove_custom_image(Request $request)
    {
        ImageManager::delete('/product/' . $request['image']);
    ...
    public function custom_delete($id)
    {
        ...
        // print_r($Fproduct);
         ...
         Cart::where('custom_product_id', $product->id)->delete();
